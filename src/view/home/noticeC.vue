...
export default {
  ...
  methods: {
    loadNews(nav_id) {
      var _self = this;
      _self.$hapi.loadNews({ nav_id:0, show: true, n_cid: parseInt(nav_id), size: _self.size, page:_self.page }).then(function(response) {
    ...
    getDetail(id){
      ...
      this.$router.push({ path: '/news/detail', query: { news_id:parseInt(id),vote:vote,backPath:'/noticeC?nav_id='+parseInt(this.$route.query.nav_id)+'&vote='+vote } });
    },
    getVoteList(){
      var _self = this;
